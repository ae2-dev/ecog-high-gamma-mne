...
d = np.load(args.work_dir / "erp.npz", allow_pickle=True)
erp_obj, erp_face = d["erp_obj"], d["erp_face"]
times = d["times"]
...
for ci, ch in enumerate(ch_names):
    X = [erp_obj[:, ci, t_sel], erp_face[:, ci, t_sel]]
    ...
    if mask.any():
        sig.append(ch)
print(f"ERP: {len(sig)} significant channel(s): {sig}")

with open(args.work_dir / "erp_stats_summary.txt", "w") as f:
    f.write(f"ERP significant channels: {sig}\n")
...
if sig:
    ...
    for ax, ch in zip(axes.ravel(), sig):
        ...
        y_obj = erp_obj[:, ci].mean(0) * 1e6
        y_face = erp_face[:, ci].mean(0) * 1e6
        ...
        ax.set(xlim=PLOT_XLIM, xlabel="time (s)", ylabel="amplitude (µV)", title=f"ERP - {ch}")
        ax.legend(loc="upper left", fontsize=8)
    for ax in axes.ravel()[len(sig):]:
        ax.set_axis_off()
    fig.tight_layout()
    out_path = args.work_dir / "05_ERP_significant_channels.pdf"
